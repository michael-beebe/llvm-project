...
module {
  func.func @stencil_boundary_exchange(%data: memref<100x100xf32>) {
    ...
    %boundary_size = arith.constant 400 : i64  // 100 * sizeof(f32)
    %left_boundary = openshmem.malloc(%boundary_size) : i64 -> !openshmem.symmetric_memref<f32>
    %right_boundary = openshmem.malloc(%boundary_size) : i64 -> !openshmem.symmetric_memref<f32>
    ...
    %transfer_size = arith.constant 100 : i64
    
    // Send left boundary to left neighbor (PE-1)
    %left_pe = arith.subi %pe, %c1 : i32
    openshmem.put(%left_boundary, %data, %transfer_size, %left_pe) : 
      !openshmem.symmetric_memref<f32>, memref<100x100xf32>, i64, i32
    
    // Send right boundary to right neighbor (PE+1)  
    %right_pe = arith.addi %pe, %c1 : i32
    openshmem.put(%right_boundary, %data, %transfer_size, %right_pe) : 
      !openshmem.symmetric_memref<f32>, memref<100x100xf32>, i64, i32
