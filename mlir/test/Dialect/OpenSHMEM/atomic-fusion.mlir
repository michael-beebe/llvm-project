...
module {
  ...
  func.func @dead_fetch_add_folds(%dest: !openshmem.symmetric_memref<i32>) {
    ...
    %r = openshmem.atomic_fetch_add(%dest, %c, %pe) : !openshmem.symmetric_memref<i32>, i32, i32 -> i32
    return
  }

  // Dead fetch folding: fetch_or -> or when result unused
  func.func @dead_fetch_or_folds(%dest: !openshmem.symmetric_memref<i64>) {
    ...
    %r = openshmem.atomic_fetch_or(%dest, %c, %pe) : !openshmem.symmetric_memref<i64>, i64, i32 -> i64
    return
  }

  // Dead fetch folding: fetch_xor -> xor when result unused
  func.func @dead_fetch_xor_folds(%dest: !openshmem.symmetric_memref<i32>) {
    ...
    %r = openshmem.atomic_fetch_xor(%dest, %c, %pe) : !openshmem.symmetric_memref<i32>, i32, i32 -> i32
    return
  }

  // Ensure we do NOT fold fetch_and (no non-fetch and op exists)
  func.func @dead_fetch_and_does_not_fold(%dest: !openshmem.symmetric_memref<i32>) {
    ...
    %r = openshmem.atomic_fetch_and(%dest, %c, %pe) : !openshmem.symmetric_memref<i32>, i32, i32 -> i32
    return
  }

  // Chain fusion over more than two adds
  func.func @fuse_add_chain_long(%dest: !openshmem.symmetric_memref<i32>) {
    %pe = arith.constant 3 : i32
    %c1 = arith.constant 4 : i32
    %c2 = arith.constant 5 : i32
    ...
    openshmem.atomic_add(%dest, %c1, %pe) : !openshmem.symmetric_memref<i32>, i32, i32
    openshmem.atomic_add(%dest, %c2, %pe) : !openshmem.symmetric_memref<i32>, i32, i32
    openshmem.atomic_add(%dest, %c3, %pe) : !openshmem.symmetric_memref<i32>, i32, i32
    return
  }

  // Chain fusion for OR with three constants
  func.func @fuse_or_chain_long(%dest: !openshmem.symmetric_memref<i32>) {
    ...
    openshmem.atomic_or(%dest, %v3, %pe) : !openshmem.symmetric_memref<i32>, i32, i32
    return
  }

  // Chain fusion for XOR with three constants
  func.func @fuse_xor_chain_long(%dest: !openshmem.symmetric_memref<i64>) {
    ...
    openshmem.atomic_xor(%dest, %v3, %pe) : !openshmem.symmetric_memref<i64>, i64, i32
    return
  }

  // Mixed add then inc -> single add, inc removed
  func.func @fuse_add_then_inc(%dest: !openshmem.symmetric_memref<i32>) {
    %pe = arith.constant 4 : i32
    %c = arith.constant 10 : i32
    // CHECK-LABEL: func.func @fuse_add_then_inc
    // CHECK: openshmem.atomic_add(
    // CHECK-NOT: openshmem.atomic_inc
    openshmem.atomic_add(%dest, %c, %pe) : !openshmem.symmetric_memref<i32>, i32, i32
    openshmem.atomic_inc(%dest, %pe) : !openshmem.symmetric_memref<i32>, i32
    return
  }

  // Mixed inc then add -> single add, inc replaced
  func.func @fuse_inc_then_add(%dest: !openshmem.symmetric_memref<i32>) {
    %pe = arith.constant 4 : i32
    %c = arith.constant 10 : i32
  ...
  func.func @no_fuse_across_quiet(%dest: !openshmem.symmetric_memref<i32>) {
    %pe = arith.constant 0 : i32
  ...
  func.func @no_fuse_different_dest(%dest1: !openshmem.symmetric_memref<i32>, %dest2: !openshmem.symmetric_memref<i32>) {
    %pe = arith.constant 0 : i32
    %c = arith.constant 1 : i32
    // CHECK-LABEL: func.func @no_fuse_different_dest
    // CHECK: openshmem.atomic_add(%arg0,
    // CHECK: openshmem.atomic_add(%arg1,
    openshmem.atomic_add(%dest1, %c, %pe) : !openshmem.symmetric_memref<i32>, i32, i32
    openshmem.atomic_add(%dest2, %c, %pe) : !openshmem.symmetric_memref<i32>, i32, i32
    return
  }

  // Negative: different PE -> no fusion
  func.func @no_fuse_different_pe(%dest: !openshmem.symmetric_memref<i32>) {
    ...
    // CHECK: openshmem.atomic_add(
    // CHECK: openshmem.atomic_add(
    openshmem.atomic_add(%dest, %c, %pe0) : !openshmem.symmetric_memref<i32>, i32, i32
    openshmem.atomic_add(%dest, %c, %pe1) : !openshmem.symmetric_memref<i32>, i32, i32
    return
  }

  // Negative: intervening side-effecting op prevents adjacency
  func.func @no_fuse_with_intervening_op(%dest: !openshmem.symmetric_memref<i32>,
